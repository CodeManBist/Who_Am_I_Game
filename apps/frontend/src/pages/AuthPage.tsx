...
import { ArrowRight, ArrowLeft, Loader2, Mail, Lock, User } from 'lucide-react';
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      if (mode === 'login') {
        ...
        const result = auth.register(username.trim(), email.trim(), password);
        if (!result.ok) {
        ...
        <div className="w-full max-w-sm animate-enter-up">
          ...
          <div className="mb-8 flex rounded-md border border-[#2A2A25] bg-[#181815] p-1">
            <button
              ...
              className={`flex-1 rounded py-2 text-sm font-semibold transition-all ${
                mode === 'login' ? 'bg-[#FF5A36] text-white' : 'text-[#9A958B] hover:text-[#F5F1E8]'
              }`}
            >
              Log in
            </button>
            <button
              ...
              className={`flex-1 rounded py-2 text-sm font-semibold transition-all ${
                mode === 'register' ? 'bg-[#FF5A36] text-white' : 'text-[#9A958B] hover:text-[#F5F1E8]'
              }`}
            >
              Register
            </button>
          ...
          <div className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Sagar"
                    className="h-12 w-full rounded-md border border-[#2A2A25] bg-[#181815] pl-10 pr-4 text-base text-[#F5F1E8] placeholder:text-[#5A564F] transition-colors focus:border-[#FF5A36]/50"
                    autoFocus
                  />
                </div>
              </div>
            ...
            <button
              ...
              disabled={loading}
              className="group flex h-12 w-full items-center justify-center gap-2 rounded-md bg-[#FF5A36] text-base font-semibold text-white transition-all hover:bg-[#ff6b4a] disabled:opacity-50 disabled:pointer-events-none"
            >
              {loading ? (
                <>
          ...
          <p className="mt-6 text-center text-sm text-[#5A564F]">
            ...
            <button
              ...
              className="font-medium text-[#FF5A36] transition-colors hover:text-[#ff6b4a]"
            >
